...
def test_notify_on_task_failure_valid(mock_post, notifyer, airflow_context):
...
@patch("src.utils.notifyer.requests.post")
def test_notify_on_task_failure_raises_if_keyerror(mock_post, notifyer):
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {"ok": True}
    mock_post.return_value = mock_response
...
@patch("src.utils.notifyer.requests.post")
def test_notify_on_task_failure_raises_if_wrong_data_format(mock_post, notifyer):
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {"ok": True}
    mock_post.return_value = mock_response
...
def test_notify_on_task_failure_if_errors_but_finally_success(
    mock_post, notifyer, airflow_context
):
    msg = "Some error"
...
@patch("src.utils.notifyer.requests.post")
def test_notify_on_task_failure_raises_if_errors(mock_post, notifyer, airflow_context):
    msg = "Some error"
    mock_post.side_effect = [
        HTTPError(msg),
        Timeout(msg),
